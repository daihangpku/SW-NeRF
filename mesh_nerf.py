...
class PathParser:
    ...
    def parse(self, experiment_config, log_checkpoint, checkpoint, checkpoint_override):
        """
        Parse the paths for the experiment configuration and model checkpoint.
        """
        if log_checkpoint:
            # Assuming the checkpoint and configuration are in the same directory
            self.config_path = os.path.join(log_checkpoint, "config.yaml")
            self.checkpoint_path = os.path.join(log_checkpoint, checkpoint if not checkpoint_override else checkpoint_override)
        else:
            self.checkpoint_path = checkpoint
            self.config_path = experiment_config
        ...
        return cfg, self.checkpoint_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log-checkpoint", type=str, default=None, help="Training log path with the config and checkpoints.")
    parser.add_argument("--checkpoint", type=str, default="model_last.ckpt", help="Path to checkpoint.")
    parser.add_argument("--save-dir", type=str, default=".", help="Directory to save the mesh.")
    parser.add_argument("--mesh-name", type=str, default="mesh.obj", help="Name of the generated mesh.")
    parser.add_argument("--iso-level", type=float, default=32, help="Iso-level for triangulation.")
    parser.add_argument("--limit", type=float, default=1.2, help="Limit for the 3D grid.")
    parser.add_argument("--res", type=int, default=128, help="Resolution of the grid for marching cubes.")
    parser.add_argument("--super-sampling", type=int, default=0, help="Super-sampling along the edges.")
    parser.add_argument("--batch-size", type=int, default=1024, help="Batch size for processing.")
    parser.add_argument("--no-view-dependence", action="store_true", default=False, help="Disable view-dependent appearance.")
    parser.add_argument("--view-disparity", type=float, default=1e-2, help="View disparity for appearance.")
    parser.add_argument("--view-disparity-max-bound", type=float, default=4e0, help="Max bound for view-dependent appearance.")
    parser.add_argument("--use-cached-mesh", action="store_true", default=False, help="Use cached mesh if available.")
    parser.add_argument("--override-cache-mesh", action="store_true", default=False, help="Override cached mesh.")
    parser.add_argument("--cache-name", type=str, default="mesh_cache.pt", help="Mesh cache file name.")
    ...
    model = getattr(model, cfg.experiment.model).load_from_checkpoint(path_parser.checkpoint_path)
